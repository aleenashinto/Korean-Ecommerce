...
// Sample pre-seeded Korean dresses dataset for Vercel Edge runtime
...
export async function POST(req: NextRequest, { params }: { params: { path: string[] } }) {
  ...
  if (pathStr === "ai/chat") {
    ...
    let reply = "✨ **Annyeonghaseyo! I'm Minji (민지), your Seoul AI Virtual Stylist.**\n\nI have handpicked our trending K-Drama and Gangnam collection dresses for you!";
    if (lastMsg.includes("tweed") || lastMsg.includes("office")) {
      reply = "✨ **Seoul Minimalist Pick**: The *Seoul Minimalist Tweed Pinafore Dress* offers crisp Gangnam boardroom elegance layered over silk blouses.";
    }
  ...
  if (pathStr === "ai/visual-search") {
    return NextResponse.json({
      visual_matches: KOREAN_DRESSES.slice(0, 4).map((p, idx) => ({
        product: p,
        visual_similarity_score: 98 - idx * 4,
        matched_attributes: ["Korean Chiffon", "Pastel Hue", "Ulzzang Cut"],
      })),
      total_matches: 4,
      analyzed_palette: "Seoul Pastel / Oat Beige",
      dominant_silhouette: "Korean Chiffon Tiered",
    });
  }

  if (pathStr === "ai/generate-copy") {
    const pName = body.product_name || "Korean Designer Dress";
    return NextResponse.json({
      luxury_description: `Straight from the fashion studios of Seoul, the **${pName}** embodies the effortless grace of Korean women's fashion. Crafted in airy Korean chiffon with delicate detailing.`,
      bullet_points: [
        "Origin: Curated Korean Atelier Fashion Edit",
        "Fabric: Premium breathable Korean Chiffon",
        "Silhouette: Flattering high-waist empire cut",
      ],
      seo_title: `${pName} | Korean Women's Dress Collection | SEOULLUXE`,
      seo_meta_description: `Shop the ${pName}. Authentic Korean dress with free express shipping & 7-day doorstep returns.`,
      social_media_caption: `Channel your inner K-Drama lead in the all-new ${pName} 🌸✨ #KFashion #SeoulStyle`,
    });
  }
